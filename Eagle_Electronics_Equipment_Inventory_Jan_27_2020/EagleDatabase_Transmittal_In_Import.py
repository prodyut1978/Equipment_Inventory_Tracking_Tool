...
def ImportTransmittalInFile():
    window = Tk()
    window.title("Transmittal In Import File Viewer")
    width = 1280
    height = 540
        ...
        if name:
            ...
            if name.endswith(tuple(extensions)):
                ...
                for sheetN in range(len(Sheet_Names)):
                    ...
                    Location = sheet.cell(row=6, column=6)
                    Location = (Location.value)
                    Date     = sheet.cell(row=5, column=3)
                    Date     = (Date.value)                
                    df = pd.read_excel(name, sheet_name=sheetN, header = None, skiprows = 10, nrows =25)
                    main_SN     = df.iloc[:,3]
                    column_names = [main_SN]
                    catdf = pd.concat (column_names, axis=1,ignore_index =True)
                    data        = pd.DataFrame(catdf)
                    data.rename(columns = {0:'main_SN'},inplace = True)
                    
                    data = data[data['main_SN'].notnull()]
    ...
    def DeleteMergedSelected():
        iDelete = tkinter.messagebox.askyesno("Delete Entry", "Confirm if you want to Delete Unknown Equipment SN")
        if iDelete >0:
            conn = sqlite3.connect("Eagle_Inventory.db")
            cur = conn.cursor()
            ImportTotalLBEntries.delete(0,END)
    ...
    btnClearAll.place(x=1194,y=470)

    L1Import = Label(window, text = "1: Import Transmittal In File", font=("arial", 10,'bold'),bg = "ghost white").place(x=1040,y=20)
    btnImportTransmittalIn = Button(window, text="Import Transmittal", font=('aerial', 9, 'bold'), height =1, width=16, bd=4, command = ImportTransmittalIn)
    btnImportTransmittalIn.place(x=1040,y=45)
    ImportTotalLBEntries  = Entry(window, font=('aerial', 12, 'bold'),textvariable= IntVar(), width = 7)
    ImportTotalLBEntries.place(x=1180,y=48)

    L2Populate = Label(window, text = "2: Merge All Empty Columns", font=("arial", 10,'bold'),bg = "ghost white").place(x=1040,y=100)
    ...
    btnPopulate.place(x=1040,y=125)
    ...
    btnDeleteMergeSelected.place(x=1175,y=128)

    L3NotMatchedSN = Label(window, text = "3: View Not Matched ManfSN", font=("arial", 10,'bold'),bg = "ghost white").place(x=1040,y=180)
    ...
    btnViewNonMatched.place(x=1040,y=205)
    NonMatchedEntries  = Entry(window, font=('aerial', 12, 'bold'),textvariable= IntVar(), width = 7)
    NonMatchedEntries.place(x=1180,y=208)

    L3UpdateDB = Label(window, text = "4. Update Import To Master DB", font=("arial", 10,'bold'),bg = "ghost white").place(x=1040,y=280)
    ...
    btnUpdateImportToDB.place(x=1040,y=305)

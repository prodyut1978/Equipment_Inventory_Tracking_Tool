...
import time
import datetime
def GenerateTransmittalOutSecondOption():
    conn = sqlite3.connect("Eagle_Inventory.db")
    TransmittalOut_df = pd.read_sql_query("SELECT * FROM Eagle_Inventory_transmittalOut2;", conn)
    ...
    ## Label and Entry
        ...
        L13 = Label(window, text = "2: To Location :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=370)    
        ToAddress  = ttk.Combobox(window, font=('aerial', 10, 'bold'),  width = 30, values= To_List)
        ToAddress.place(x=1010,y=370)

        L14 = Label(window, text = "3: Other To/From :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=410)    
        OtherToFrom  = Entry(window, font=('aerial', 10, 'bold'),textvariable = StringVar(), width = 33, bd=2)
        OtherToFrom.place(x=1010,y=410)

        L15 = Label(window, text = "4: Transported By :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=450)
        VehicalNumber  = Entry(window, font=('aerial', 10, 'bold'),textvariable = StringVar(), width = 33, bd=2)
        VehicalNumber.place(x=1010,y=450)

        L16 = Label(window, text = "5: Receiver Name :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=490)
        ReceiverName  = Entry(window, font=('aerial', 10, 'bold'),textvariable = StringVar(), width = 33, bd=2)
        ReceiverName.place(x=1010,y=490)

        L17 = Label(window, text = "6: Shipper Name :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=530)
        ShipperName  = Entry(window, font=('aerial', 10, 'bold'),textvariable = StringVar(), width = 33, bd=2)
        ShipperName.place(x=1010,y=530)

        L18 = Label(window, text = "7: PO Number :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=570)
        PONumber  = Entry(window, font=('aerial', 10, 'bold'),textvariable = StringVar(), width = 33, bd=2)
        PONumber.place(x=1010,y=570)

        L19 = Label(window, text = "G: Generated Transmittal Equipment Information :", font=("arial", 12,'bold'),bg = "ghost white").place(x=840,y=620)
                          
        L20 = Label(window, text = "1: Equipment Category :", font=("arial", 10,'bold'),bg = "ghost white").place(x=840,y=650)
        ...
        def DeleteSelectData():
            ...
            if iDelete >0:
                ...
                for selected_item in tree.selection():
                    cur.execute("DELETE FROM Eagle_Inventory_transmittalOut2 WHERE main_SN =? ", (tree.set(selected_item, '#4'),))
                    conn.commit()
